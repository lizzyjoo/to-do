...
import { isToday, parseISO, startOfDay, isBefore, isFuture } from "date-fns";
...
class UI {
  ...
  static addEventListeners() {
    ...
    this.anytimeCheck.addEventListener("change", () =>
      this.handleAnytimeCheckbox()
    );
    this.dueDateInput.addEventListener("input", () => this.handleDueInput());
    this.addProjectBtn.addEventListener("click", () =>
      this.displayProjectModal()
    );
    this.projectModalClose.addEventListener("click", () =>
    ...
    this.defaultProjects.forEach((project) => {
      project.addEventListener("click", (event) =>
        this.handleDefaultProjectClick(event)
      );
    });
    ...
    if (taskId) {
      ...
      if (task) {
        ...
        deleteButton.addEventListener("click", () =>
          this.handleDeleteTaskButtonClick(task, taskEditModal, taskItem)
        );
        ...
        taskForm.addEventListener("submit", (event) => {
          this.handleTaskEditFormSubmit(event, task, taskForm, taskItem);
        });
      }
    }
  }
  ...
  static handleDeleteTaskButtonClick(task, taskEditModal, taskItem) {
    ...
    projects.forEach((project) => {
      ...
      if (taskInProject) {
        // Remove from all projects except 'Inbox'
    ...
    this.loadProjectPage(
      this.taskSection
        .querySelector(".page-header .project-header-title")
  ...
  static setupEditTaskForm(task, taskForm, taskItem) {
    ...
    deleteButton.addEventListener("click", () =>
      this.handleDeleteTaskButtonClick(task.id)
    );
  ...
  static handleCheckboxChange(event, taskId) {
    ...
    if (taskDiv) {
      if (isChecked) {
        ...
        const completedProjectDiv = document.querySelector(
          '.task-div[data-value="5"]'
        );
        ...
        const originalProjectDiv = document.querySelector(
          `.task-div[data-value="${task.parentProject}"]`
        );
  ...
  static handleTaskEditFormSubmit(event, task, taskForm, taskItem) {
    ...
    if (!this.dueDateInput.value && !this.anytimeCheck.checked) {
      this.errorMessageDiv.textContent =
        "Please provide either a due date or check 'Anytime'.";
    ...
    if (taskForm.checkValidity()) {
      ...
      this.updateTaskDueDateAndProject(
        task,
        newDue,
        anytime,
        taskPrevParentProjectID,
        projects
      );

      // Update the task in all projects
      this.updateTaskInProjects(
        task,
        newContent,
        newPriority,
        newDue,
      ...
      const currentProjectId = this.taskSection
        .querySelector(".page-header .project-header-title")
        .getAttribute("data-project-id");
      ...
      this.resetForm(
        taskForm.closest(".task-edit-modal").querySelector(".close-modal")
      );
      taskForm.closest(".task-edit-modal").style.display = "none";
    }
  }

  static updateTaskDueDateAndProject(
    task,
    newDue,
    anytime,
    taskPrevParentProjectID,
    ...
    const updateTaskAndProject = (newDue, anytime) => {
      ...
      const prevProject = projects.find(
        (proj) => proj.id === taskPrevParentProjectID
      );
      ...
      if (anytimeProject) {
        const taskIndex = anytimeProject.tasks.findIndex(
          (t) => t.id === task.id
        );
        if (taskIndex !== -1) {
      ...
      if (upcomingProject) {
        const taskIndex = upcomingProject.tasks.findIndex(
          (t) => t.id === task.id
        );
        if (taskIndex !== -1) {
      ...
      if (overdueProject) {
        const taskIndex = overdueProject.tasks.findIndex(
          (t) => t.id === task.id
        );
        if (taskIndex !== -1) {
  ...
  static updateTaskInProjects(
    ...
    projects.forEach((project) => {
      ...
      if (taskInProject) {
        ...
        if (
          project.id === taskPrevParentProjectID &&
          project.id !== "1" &&
          project.id !== "4"
        ) {
          project.removeTask(task);
  ...
  static addProjectToTaskForm(project) {
    ...
    const taskFormParentProject =
      this.taskForm.querySelector("#parent-project");
  ...
  static createTaskDiv(task, currentProjectID) {
    ...
    taskCheckBox.addEventListener("change", (event) =>
      this.handleCheckboxChange(event, task.id)
    );
    ...
    if (
      task.parentProject !== currentProjectID &&
      !["1", "2", "3", "4", "5", "6"].includes(project.id)
    ) {
    ...
    taskLabels.addEventListener("click", () =>
      this.handleTaskItemClick(taskItem)
    );
  ...
  static createProjectDiv(project) {
    ...
    editOption.addEventListener("click", () =>
      this.handleEditOption(project.id, projectItemDiv)
    );
    deleteOption.addEventListener("click", () =>
      this.handleDeleteOption(project, projectItemDiv)
    );
  ...
  static resetForm(closeModalBtn) {
    const modal =
      closeModalBtn.closest(".project-modal") ||
      closeModalBtn.closest(".task-modal") ||
      closeModalBtn.closest(".project-edit-modal");
    if (modal) {
  ...
  static handleEditFormSubmit(event, projectEditForm, projectItemDiv, project) {
    ...
    if (projectEditForm.checkValidity()) {
      ...
      const newTitle = projectEditForm.querySelector(
        "#project-title-edit"
      ).value;
      const newDescription = projectEditForm.querySelector(
  ...
  static updateTaskFormParentProject(project) {
    ...
    const originalParentOption = formParentSelectElem.querySelector(
